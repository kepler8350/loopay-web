...
async function updateMatchingBtn(preloadedCt){
  try{
    ...
    if(btn1){ 
      ...
      var _r1Done = _r1RanServer || window._r1MatchingDone;
      var _active1 = isActive1 && !_block1Hidden && !_r1Done;
      btn1.disabled = !_active1;
      btn1.style.opacity = _active1 ? '1' : '0.45';
      btn1.style.cursor = _active1 ? 'pointer' : 'not-allowed';
      btn1.textContent = _r1Done ? '✅ 1차 매칭 완료' : '⚡ 1차 매칭 실행하기';
    }
    if(notice){
...
async function loadReservationsLog(page){
  ...
  try{
    ...
    if(tbody) tbody.innerHTML = rows.map(function(r, i){
      ...
      // 2차대기 조건: pending + join_round2=1 + ms=null + 1차매칭실행됨
      if(r.status==='pending' && r.res_type==='buy' && r.join_round2===1 && !r.match_status){
        var _rDate = r.reserve_date;
        var _isToday2 = (_rDate === _today2);
        if(!_isToday2 || (_isToday2 && _r1RanToday)){
          statusKo = '2차대기';
        }
      }
...
async function loadMatchingStatus(){
  ...
  try{
    ...
    var _r2SellCount = _r2Done ? 0 : ((d.r2_sell_count!=null) ? d.r2_sell_count : (r2.sell_count!=null?r2.sell_count:0));
    // cnt → count 필드명 정규화
...
async function openLuckyBuy(){
  ...
  try{
    ...
    if(d.success&&d.pairs){
      ...
      var reasons=[];
      if(maxBz===0&&d.pairs.bronze&&d.pairs.bronze.reason) reasons.push('수정: '+d.pairs.bronze.reason);
      if(maxSv===0&&d.pairs.silver&&d.pairs.silver.reason) reasons.push('루비: '+d.pairs.silver.reason);
      if(maxGd===0&&d.pairs.gold&&d.pairs.gold.reason) reasons.push('다이아: '+d.pairs.gold.reason);
