...
function rlToggleAll(cb){
  document.querySelectorAll('.rl-check').forEach(el => { el.checked = cb.checked; });
  rlUpdateDelBtn();
}
...
async function deleteSelectedReservations(){
  ...
  var ids = Array.from(checked).map(el => parseInt(el.dataset.id));
  if(!confirm(ids.length+'건의 예약기록을 삭제하시겠습니까?')) return;
  try{
    var d = await apiAdmin('/admin/reservations/delete',{method:'POST',body:JSON.stringify({ids})});
