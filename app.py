...
def _do_confirm_transfer(db, m):
    ...
    try:
        ...
        # seller_item이 이미 sold이면 이전에 처리됨
        _existing = None
        if dict(m).get('seller_item_id'):
            _si_check = db.execute(
                "SELECT id FROM items WHERE id=? AND status='sold'",
                (m['seller_item_id'],)
            ).fetchone()
            if _si_check:
                _existing = _si_check
        ...
        if seller_item:
            ...
            _today = get_today().isoformat()
            if _buyer_is_loopay:
                # 루페이 구매 시: buyer_res_id → 예약/아이템 조회 (단계별 처리)
                ...
                if _buyer_res_id:
                    ...
                    if not _loopay_item_updated:
                        # 2) buyer_res_id가 아이템 id인 경우 (루페이 예약 없이 자동 구매 시)
                        rows_updated = db.execute(
                            "UPDATE items SET stage=?, status='reservable', purchase_date=? WHERE id=? AND user_id=? AND status='matched'",
                            (_stage, _today, _buyer_res_id, loopay_id)
                        ).rowcount
                        if rows_updated > 0:
                            _loopay_item_updated = True
                if not _loopay_item_updated:
                    # 3) seller_item과 같은 bar_type의 loopay matched 아이템 찾기
        ...
        if match_round == 1:
            ...
            if _loopay_row:
                ...
                if _item:
                    ...
                    _bar = _item['bar_type']
                    # 중복 방지: seller_item_id에 대해 loopay 구매 매치가 이미 있으면 skip
...
@app.route('/api/admin/testtools/set-match-status', methods=['POST'])
@jwt_required()
def testtools_set_match_status():
    identity = get_jwt_identity()
    if not str(identity).startswith('admin:'): return jsonify(error='forbidden'), 403
    data = request.json or {}
    db = get_db()
            ...
            for piece in pieces:
                for _ in range(piece.get('count', 1)):
                    ...
                    db.execute(
                        ...
                        (lid, new_item_id, bar, actual_round, today, piece['stage'], join_round2)
                    )
            db.commit()
            return jsonify(success=True, split=True, message=f'분할 판매예약 완료 ({len(pieces)}종)')
        else:
            db.execute("UPDATE items SET status='reservable' WHERE id=?", (item_id,))
            db.execute(
                "INSERT INTO reservations(user_id, item_id, bar_type, match_round, reserve_date, status, stage, confirmed, join_round2) VALUES(?,?,?,?,?,'pending',?,1,?)",
                (lid, item_id, bar, actual_round, today, stage, join_round2)
            )
            db.commit()
